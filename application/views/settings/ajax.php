...
$("#serverGuruSwitch").click(function() {
  ...
  $.ajax({
    type: 'POST',
    url: "<?= base_url('settings/switchServerGuru'); ?>",
    data: formServerGuru,
    cache: false,
    success: function(data) {
...
$("#serverSiswaSwitch").click(function() {
  ...
  $.ajax({
    type: 'POST',
    url: "<?= base_url('settings/switchServerSiswa'); ?>",
...
$("#serverWebSwitch").click(function() {
  var formServerWeb = $('#formSwitchServerWeb').serialize();
  var statusServerWeb = document.getElementById("statusServerWeb").value;
  $.ajax({
    type: 'POST',
    url: "<?= base_url('settings/switchServerWeb'); ?>",
    data: formServerWeb,
    cache: false,
    success: function(data) {
      if (statusServerWeb == 0) {
        setTimeout(function() {
          toastr['success'](
            'Web dapat diakses kembali !',
            'Server Web Aktif !', {
              closeButton: true,
              tapToDismiss: true
            }
        ...
        document.getElementById("statusServerWeb").value = "1"
        document.getElementById("serverWebSwitchLabel").innerHTML = "Aktif";
      } else {
        setTimeout(function() {
          toastr['error'](
            'Web tidak dapat diakses !',
            'Server Web Tidak Aktif !', {
              closeButton: true,
              tapToDismiss: true
            }
          );
        }, 0);
        document.getElementById("statusServerWeb").value = "0"
        document.getElementById("serverWebSwitchLabel").innerHTML = "Tidak Aktif";
      }
    }
  });
});
