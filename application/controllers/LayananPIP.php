...
class LayananPIP extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('App_model', 'modelApp');
		$this->load->model('PIP_Model', 'ModelPIP');
		$this->load->helper('pip_helper', 'pipHelper');
	...
	public function index()
	{
		...
		$data['page'] = "Layanan PIP";

		$this->load->view('templates/header', $data);
		$this->load->view('templates/navbar', $data);
		$this->load->view('gtk/pip/dashboard', $data);
		$this->load->view('templates/modal', $data);
		$this->load->view('templates/footer', $data);
		$this->load->view('gtk/pip/ajax', $data);
	}
	...
	public function switchAccess()
	{
		...
		$checkData = $this->db->get_where('pip_tapel', ['id' => $data['id']]);
		$row = $checkData->row_array();
		if ($checkData->num_rows() == "1") {
			if ($data['is_active'] == "0") {
				$query = "UPDATE `pip_tapel` SET `is_active` = '0'";
				$this->db->query($query);
				$this->db->set('is_active', '1');
				$this->db->where('id', $data['id']);
				$this->db->update('pip_tapel');
				$this->session->set_flashdata('toastr', "
        <script>
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Hak Akses PIP Tahun Pelajaran " . $row['tapel'] . " Di Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Hak Akses PIP Tahun Pelajaran " . $row['tapel'] . " Di Non-Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
      ...
      $(window).on('load', function() {
        setTimeout(function() {
          toastr['error'](
            'PIP Tahun Pelajaran " . $row['tapel'] . " Tidak Tersedia !',
            'Gagal !', {
              closeButton: true,
              tapToDismiss: true
            }
	...
	public function switchRegistrasi1()
	{
		...
		if ($checkData->num_rows() == "1") {
			if ($data['is_active_reg1'] == "0") {
				$query = "UPDATE `pip_tapel` SET `is_active_reg1` = '0'";
				$this->db->query($query);
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Registrasi PIP Tahun Pelajaran " . $row['tapel'] . " Di Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Registrasi PIP Tahun Pelajaran " . $row['tapel'] . " Di Non-Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
	...
	public function switchRegistrasi2()
	{
		...
		if ($checkData->num_rows() == "1") {
			if ($data['is_active_reg2'] == "0") {
				$query = "UPDATE `pip_tapel` SET `is_active_reg2` = '0'";
				$this->db->query($query);
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Daftar Ulang PIP Tahun Pelajaran " . $row['tapel'] . " Di Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Daftar Ulang PIP Tahun Pelajaran " . $row['tapel'] . " Di Non-Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
	...
	public function switchResult()
	{
		...
		if ($checkData->num_rows() == "1") {
			if ($data['is_active_result'] == "0") {
				$query = "UPDATE `pip_tapel` SET `is_active_result` = '0'";
				$this->db->query($query);
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Pengumuman PIP Tahun Pelajaran " . $row['tapel'] . " Di Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
        ...
        $(window).on('load', function() {
          setTimeout(function() {
            toastr['success'](
              'Pengumuman PIP Tahun Pelajaran " . $row['tapel'] . " Di Non-Aktifkan !',
              'Berhasil !', {
                closeButton: true,
                tapToDismiss: true
              }
	...
	public function settings()
	{
		...
		$this->load->view('gtk/pip/settings', $data);
		$this->load->view('templates/modal', $data);
		$this->load->view('templates/footer', $data);
		$this->load->view('gtk/pip/ajax', $data);
	}
	...
	public function tambahTapel()
	{
		...
		$checkData = $this->db->get_where('pip_tapel', ['tapel' => $data['tapel']]);
		if ($checkData->num_rows() == "0") {
			$this->db->insert('pip_tapel', $data);
			$this->session->set_flashdata('toastr', "
      <script>
      $(window).on('load', function() {
        setTimeout(function() {
	...
	public function deleteTapel()
	{
		...
		$checkData = $this->db->get_where('pip_tapel', ['id' => $data['id']]);
		if ($checkData->num_rows() == "1") {
			$this->db->delete('pip_tapel', ['id' => $data['id']]);
	...
	public function setUp()
	{
		...
		is_pip_exist($data['url_param']);

		$data['pageCollumn'] = "1-column";
		$data['page'] = "PIP " . $data['url_param'];
		$this->load->view('templates/header', $data);
		$this->load->view('templates/navbar', $data);
		$this->load->view('gtk/pip/setup', $data);
		$this->load->view('templates/modal', $data);
		$this->load->view('templates/footer', $data);
		$this->load->view('gtk/pip/ajax', $data);
	}
	...
	function setupLoad()
	{
		...
		$data['persuratan'] = $this->ModelPIP->getPersuratan($url_param)->row_array();
		$kepalaSekolah = $data['persuratan']['kepalaSekolah'];
		$data['kepalaSekolah'] = $this->ModelPIP->getKepalaSekolah($kepalaSekolah)->row_array();
		$data['page'] = $url_param;
	...
	function editPersuratan()
	{
		...
		$this->db->where('username', $pipPersuratan['kepalaSekolah']);
		$this->db->update('profil_gtk');

		$this->db->set($pipPersuratan);
		$this->db->where('id', $pipPersuratan['id']);
		$this->db->update('pip_tapel');
		$this->session->set_flashdata('toastr', "
    <script>
    $(window).on('load', function() {
      setTimeout(function() {
        toastr['success'](
          'Persuratan PIP Tahun Pelajaran " . $tapel . " telah diperbarui !',
          'Berhasil !', {
            closeButton: true,
            tapToDismiss: true
          }
		...
		redirect(base_url('LayananPIP/SetUp/') . $url_param);
	}
	...
	function tambahPanitia()
	{
		...
		$checkData = $this->db->get_where('pip_panitia', ['tapel' => $data['tapel'], 'username' => $data['username']]);
		if ($checkData->num_rows() == "0") {
			$this->db->insert('pip_panitia', $data);
			$this->session->set_flashdata('toastr', "
      <script>
      $(window).on('load', function() {
        setTimeout(function() {
          toastr['success'](
            'Panitia PIP Tahun Pelajaran " . $data['tapel'] . " Di Tambahkan !',
            'Berhasil !', {
              closeButton: true,
              tapToDismiss: true
            }
      ...
      $(window).on('load', function() {
        setTimeout(function() {
          toastr['error'](
            'Panitia PIP Tahun Pelajaran " . $data['tapel'] . " Sudah Terdaftar !',
            'Gagal !', {
              closeButton: true,
              tapToDismiss: true
            }
	...
	public function deletePanitia()
	{
		...
		$checkData = $this->db->get_where('pip_panitia', ['username' => $data['username']]);
		if ($checkData->num_rows() == "1") {
			$this->db->delete('pip_panitia', ['username' => $data['username']]);
			$response['status'] = 'success';
			$response['judul'] = 'Berhasil !';
	...
	{
		$tapel = htmlspecialchars($this->input->post('tapel', true));
		$query = "SELECT `username` FROM `pip_panitia` WHERE `tapel` = '$tapel'";
		$queryPanitia = $this->db->query($query)->result_array();
		foreach ($queryPanitia as $rowUsername) {
			$this->db->delete('pip_panitia', ['username' => $rowUsername['username']]);
		}
		$response['status'] = 'success';
		$response['judul'] = 'Berhasil !';
